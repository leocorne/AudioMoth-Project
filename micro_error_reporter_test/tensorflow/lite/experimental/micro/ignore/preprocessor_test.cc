...
TF_LITE_MICRO_TEST(TestPreprocessor) {
  ...
  //error_reporter->Report("Begin test");

  uint8_t yes_calculated_data[g_yes_power_spectrum_data_size];
  //error_reporter->Report("Data to be processed is %d bytes", g_yes_power_spectrum_data_size);
  ...
  //error_reporter->Report("Preprocessing yes OK. Comparing values now");
  ...
  //error_reporter->Report("Comparison for Yes OK");
  uint8_t no_calculated_data[g_no_power_spectrum_data_size];
  ...
  //error_reporter->Report("Preprocessing no OK. Comparing values now");
  ...
  //error_reporter->Report("Comparison for No OK. End of tests");
}
